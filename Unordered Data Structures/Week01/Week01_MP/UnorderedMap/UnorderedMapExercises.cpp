...
StringIntMap makeWordCounts(const StringVec& words) {
  ...
  for( int i = 0; i < len; i++){
    ...
    if(wordcount_map.count(cur)){
      //string cur exists
      int num = wordcount_map[cur];
      wordcount_map[cur] =  num + 1;
    } else {
      //string cur not exists
      wordcount_map[cur] = 1;
    }
  }
...
int memoizedLongestPalindromeLength(LengthMemo& memo, const std::string& str, 
    int leftLimit, int rightLimit, timeUnit startTime, double maxDuration) {
