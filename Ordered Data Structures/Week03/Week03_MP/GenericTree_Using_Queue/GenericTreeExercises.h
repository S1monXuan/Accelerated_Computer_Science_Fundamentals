...
static void treeFactory(GenericTree<int>& tree) {
  ...
  tree.clear();
  //GenericTree<int>::TreeNode* newNode = tree.getRootPtr();

  tree.createRoot(4);
  GenericTree<int>::TreeNode* node8 = tree.getRootPtr()->addChild(8);
  tree.getRootPtr()->addChild(15);
  GenericTree<int>::TreeNode* node16 = node8->addChild(16);
  node8->addChild(23);
  node16->addChild(42);
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  std::queue<TreeNode*> childQueue;
  childQueue.push(rootNodePtr);
  int len = 0;
  TreeNode *temp;
  while(!childQueue.empty()){
    len = childQueue.size();
    for(int i = 0; i < len; i++){
      temp = childQueue.front();
      childQueue.pop();
      results.push_back(temp->data);
      for (auto childPtr : temp->childrenPtrs) {
        childQueue.push(childPtr);
      }
    }
  }
  return results;
